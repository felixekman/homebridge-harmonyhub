...
function HomePlatform(log, config, api) {
	...
	self._ignoredNames = config.ignored_names ? config.ignored_names : [];
	...
	if (doRegister) {
		task = task
			.tap(function(accList) {
				if (!self._api) return;
				self.log(util.inspect(accList, false, null));
				accList = _.filter(accList, function(acc) {
					return acc && !(self._ignoredNames.includes(acc.displayName));
				});
