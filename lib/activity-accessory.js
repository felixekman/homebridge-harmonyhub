...
function ActivityAccessory(accessory, log, connection) {
    ...
    if (this.accessory.getService(Service.Speaker) === undefined) {
    ...
    this.accessory
        ...
        .on('set', function(value, callback) {
            if (!this._muteControl) {
                callback(null);
                return;
            }

            var action = 'action=' +  this._muteControl.action.replace(/\:/g, '::');

            this.connection.client.send("holdAction", action + ':status=press').then(function() {
                this.connection.client.send("holdAction", action + ':status=release');
            }.bind(this));
...
ActivityAccessory.prototype.initAsync = function() {
	...
	var self = this;
	self.log("Ignored names: \n"+self._ignoredNames.map(function (a) {
			return "\t" + a;
		}).join("\n"));
	return Promise.all([
		...
		.spread(function (activities, currentActivity) {
			...
			activities = _.filter(activities, function(acc) {
				return acc && !(self._ignoredNames.includes(acc.label));
			});
			self.log("Filtered activities: \n" + activities.map(function (a) {
					return "\t" + a.label;
				}).join("\n"));
...
ActivityAccessory.prototype._updateActivities = function(list) {
    ...
    var volumeControls = list[0].controlGroup
        .filter(function (group) { return group.name.toLowerCase() === 'volume' })
        .pop();

    if (volumeControls) {
        _.forEach(volumeControls.function, function(fn) {
            if (fn.name === "Mute") {
                this._muteControl = fn;
            }
        }.bind(this));
    }
